...
&emsp;&emsp;Hi-Fi RUSH是个玩起来非常爽快的动作游戏，这个游戏的战斗有个非常经典的机制也就是格挡。第三关的时候旁白会教你该如何格挡，虽然看起来很简单只要在闪光（发出声音）后按E即可，但是在我进行全关卡皇冠S通关流程时，我却发现实际上格挡的要求其实还挺严格的。于是我想逆向分析一下，实现自动格挡。

# 分析

&emsp;&emsp;首先有个函数一定是和主角的格挡判断有关，在这个游戏中格挡成功与否与是否减少生命值有着直接的联系，有了这个我们就可以开始了。通过查找生命值的增加与减少来找到储存生命值的地址，我发现格挡失败时实现生命值减少的函数在这里。
...
&emsp;&emsp;通过不断下**断点**加上**Step Out**测试是否和格挡判断函数有关，我们最终来到了这个冗长的函数，先省略其它的无关指令。
...
Hi-Fi-RUSH.exe+193230B - E9 90FCFFFF           - jmp Hi-Fi-RUSH.exe+1931FA0 { 重要跳转 }
...
&emsp;&emsp;在这个函数入口处下断点，当敌人攻击时游戏会直接暂停。进一步使用**Step Out**并且下断点调试时，游戏会在菜单界面直接暂停。所以上面提到的函数一定和格挡判断函数有关，已知在```Hi-Fi-RUSH.exe+193274F call Hi-Fi-RUSH.exe+1932BD0```处会执行和生命值减少有关的函数，那么前面的指令一定有**jmp**指令或**ret**指令。

&emsp;&emsp;经过单步调试发现，程序在```Hi-Fi-RUSH.exe+19322CB call qword ptr [rax+000008F8]```处执行了```rax+000008F8```所指向的函数，接着会使用指向的函数设置的**al**的值在```Hi-Fi-RUSH.exe+19322D3 - 74 3B                 - je Hi-Fi-RUSH.exe+1932310```处判断是否要跳转至```Hi-Fi-RUSH.exe+1932310```。如果格挡成功了程序不会跳转，会一直执行到```Hi-Fi-RUSH.exe+193230B - E9 90FCFFFF           - jmp Hi-Fi-RUSH.exe+1931FA0 { 重要跳转 }```处的**jmp**指令。如果格挡失败了程序会跳转至```Hi-Fi-RUSH.exe+1932310```处，从而执行后面的和生命值减少有关的指令。

&emsp;&emsp;其实这里有点感觉了，这里的```Hi-Fi-RUSH.exe+19322CB call qword ptr [rax+000008F8]```应该是多态的一个表现。当我进行测试时，发现主角和机器人被攻击时**rax**寄存器的值会变化，但是当主角受到攻击时这里的**rax**总是个定值。通过**Step In**调试，最终程序执行到了这个函数。
...
&emsp;&emsp;这里比较了两个浮点数然后来判断格挡是否成功，感觉应该是和格挡时机有关，在游戏教学中旁白告诉了玩家要在拍点时按下格挡键才能成功格挡。到这个地方就分析完了，不过我还观察了下从```Hi-Fi-RUSH.exe+1932317 - FF 90 E8080000        - call qword ptr [rax+000008E8] { 闪避判断函数 }```到```Hi-Fi-RUSH.exe+1932365 - 49 8B 7F 30           - mov rdi,[r15+30]```之间的代码块，发现和格挡判断的代码块很像，直觉告诉我```[rax+000008E8]```是闪避判断函数，修改这个函数后主角也确实会在处决过程中百分之百闪避。接下来可以写个程序来修改汇编指令，让主角永远完美格挡机器人的攻击。
